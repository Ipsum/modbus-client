...
class toplevels:
    ...
    def comset(self, master):
        ...
        self.rb3 = Radiobutton(w1, text="38400", variable=self.br, value=2)
        self.rb4 = Radiobutton(w1, text="57600", variable=self.br, value=3)
        self.rb1.grid(row=2, column=1)
        self.rb2.grid(row=3, column=1, sticky=N)
        self.rb3.grid(row=4, column=1, sticky=N)
        self.rb4.grid(row=5, column=1, sticky=N)
        self.rb1.bind('<1>', self.rb9600) #trigger of left mouse button
        self.rb2.bind('<1>', self.rb19200)
        self.rb3.bind('<1>', self.rb38400)
        self.rb4.bind('<1>', self.rb57600)
        util.BAUDRATE = [9600,19200,38400,57600][default["br"]]
        ...
        self.retreive.grid(row=12,columnspan=2,sticky=E+W,pady=5)
        self.default1 = Button(w2, text="Defaults 1", command=self.defaultsone)
        self.default1.grid(row=13,columnspan=2,sticky=E+W,pady=5)
        self.default2 = Button(w2, text="Defaults 2", command=self.defaultstwo)
        self.default2.grid(row=14,columnspan=2,sticky=E+W,pady=5)
...
By: {1}'''.format(__version__,__author__),'About')
    ...
    def rb19200(self,master):
        util.BAUDRATE = 19200
        return
    def rb38400(self,master):
        util.BAUDRATE = 38400
        return
    def rb57600(self,master):
        util.BAUDRATE = 57600
        return
    ...
    def defaultsone(self):
        """set defaults 1 on the meter"""
        util.errlvl=4 #error on first error
        resp = 0
        s['port'] = int(self.com.get()[-1])-1
        ser = modbus.openConn(s)
        if ser:
            try:
                resp = modbus.writeReg(ser,modbus.fc['reset'],int(modbus.reg['defaults one']),0)
                ser.close()
            except:
                return False
